...
func blurhashDecodingIntoDataTests(
    hash: String,
    width: Int,
    height: Int,
    sampleFilename: String
) async throws {
    let data = Data(blurhash: hash, width: width, height: height)! // <-- tested
...
func blurhashDecodingIntoUnsafeRawBufferPointerTests(
    hash: String,
    width: Int,
    height: Int,
    sampleFilename: String
) async throws {
    let data = UnsafeRawBufferPointer(blurhash: hash, width: width, height: height)! // <-- tested
        .withUnsafeBytes { ptr in                             // Adds alpha channel
            var data = Data()                                 // because Bitmap needs it
            data.reserveCapacity(ptr.count / 3 * 4)           //
            for i in stride(from: 0, to: ptr.count, by: 3) {  //
